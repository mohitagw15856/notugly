...
import { chance, hash } from './seed.mjs';
import { hexToOklch, oklchToHex, toDisplayable, contrast, accessibleOn, isLight } from './color.mjs';
import { dog, duck, capybara, monster, object, line, duo } from './creatures.mjs';

export const STYLES = [
  'face', 'pencil', 'specs', 'cat', 'ghost', 'sticker', 'line',
  'dog', 'duck', 'capybara', 'monster', 'object', 'duo',
  'geometric', 'blob', 'pixel', 'rings', 'initials', 'isometric', 'bauhaus',
];

// Moods change the expression without changing the person. Same seed, same
// face, different day — which is what you want for presence indicators.
export const MOODS = ['neutral', 'happy', 'thinking', 'surprised', 'sleepy', 'determined'];

// Only the styles with a drawn face can hold an expression. Saying so out loud
// beats silently ignoring the option.
export const MOOD_STYLES = ['face', 'specs', 'pencil'];

// Hats are drawn over the head, so they need to know where the head is. These
// are the crown coordinates for each style that has one.
const HEADS = {
  face: [50, 26, 26], specs: [50, 22, 30], pencil: [50, 19, 19],
  cat: [50, 32, 27], ghost: [50, 30, 22], dog: [50, 31, 26],
  capybara: [50, 38, 26], monster: [50, 28, 26], duck: [58, 21, 17],
};

export const HATS = ['none', 'party', 'beanie', 'shades', 'scarf', 'halo', 'crown'];
...
// What a mood does to a face. Kept in one table so `face`, `specs` and `pencil`
// can't drift into disagreeing about what "sleepy" looks like.
const EXPRESSION = {
  happy: { mouth: 'grin', blink: false, brow: true, browTilt: 6, eyeScale: 1.05 },
  thinking: { mouth: 'smirk', blink: false, brow: true, browTilt: -9, eyeScale: 0.95 },
  surprised: { mouth: 'ooh', blink: false, brow: true, browTilt: 10, eyeScale: 1.35 },
  sleepy: { mouth: 'flat', blink: true, brow: false, browTilt: 0, eyeScale: 1 },
  determined: { mouth: 'flat', blink: false, brow: true, browTilt: -12, eyeScale: 0.9 },
};

function face(seed, p, _label, _backdrop, opts = {}) {
  const c = chance(`face:${seed}`);
  const mood = EXPRESSION[opts.mood] || null;
  const eyeY = c.int(44, 50);
  const eyeGap = c.int(14, 20);
  // Draw every value from the stream first, then let the mood overrule it.
  // Skipping a draw would shift every later one, and you'd get a different
  // person rather than the same person in a different frame of mind.
  const baseEyeR = c.float(3.4, 5.6);
  const baseBlink = c.bool(0.18);
  const baseBrow = c.bool(0.45);
  const baseTilt = c.float(-8, 8);
  const baseMouth = c.weighted([['smile', 5], ['grin', 3], ['flat', 2], ['ooh', 2], ['smirk', 2]]);

  const eyeR = baseEyeR * (mood?.eyeScale ?? 1);
  const blink = mood ? mood.blink : baseBlink;
  const brow = mood ? mood.brow : baseBrow;
  const browTilt = mood ? mood.browTilt : baseTilt;
  const mouth = mood ? mood.mouth : baseMouth;
...
function pencil(seed, p, _label, _backdrop, opts = {}) {
  ...
  return `
    ...
    ${mood
      ? {
          happy: `<path d="${wobble(c, [[45, headY + 6], [50, headY + 11], [55, headY + 6]], 1)}" ${stroke}/>`,
          thinking: `<path d="${wobble(c, [[45, headY + 9], [55, headY + 6]], 1)}" ${stroke}/>`,
          surprised: `<path d="${sketchCircle(c, 50, headY + 8, 3.5, 0.8)}" ${stroke}/>`,
          sleepy: `<path d="${wobble(c, [[45, headY + 8], [55, headY + 8]], 1)}" ${stroke}/>`,
          determined: `<path d="${wobble(c, [[44, headY + 8], [56, headY + 8]], 0.6)}" ${stroke}/>`,
        }[opts.mood]
...
// its glasses.
function specs(seed, p, _label, _backdrop, opts = {}) {
  ...
  const baseEyeR = c.float(2.6, 3.6);
  const baseBlink = c.bool(0.16);
  const baseMouth = c.weighted([['smile', 5], ['line', 2], ['oh', 2], ['grin', 3]]);

  // `specs` names its flat mouth "line" and its round one "oh"; the shared
  // table speaks in `face`'s vocabulary, so translate rather than duplicate.
  const SAY = { flat: 'line', ooh: 'oh', smirk: 'line', smile: 'smile', grin: 'grin' };
  const eyeR = baseEyeR * (mood?.eyeScale ?? 1);
  const blink = mood ? mood.blink : baseBlink;
  const mouth = mood ? SAY[mood.mouth] : baseMouth;
...
// --- hats --------------------------------------------------------------------
// Drawn last, on top of the head, anchored per style. A hat is the cheapest way
// to make one seed feel seasonal without breaking the "same person forever"
// promise — the face underneath is untouched.

function hatFor(kind, seed, p, style) {
  const anchor = HEADS[style];
  if (!anchor || kind === 'none' || !kind) return '';
  const [x, y, r] = anchor;
  const c = chance(`hat:${kind}:${seed}`);
  const col = c.pick([p.accent, p.mid]);

  const hats = {
    party: `<path d="M${x} ${y - 26} L${x - 13} ${y + 3} L${x + 13} ${y + 3} Z" fill="${col}"/>
            <circle cx="${x}" cy="${y - 26}" r="4" fill="${p.light}"/>
            <path d="M${x - 9} ${y - 6} h18 M${x - 11} ${y - 1} h22" stroke="${p.light}" stroke-width="2" opacity=".8"/>`,
    beanie: `<path d="M${x - r} ${y + 6} a${r} ${r} 0 0 1 ${r * 2} 0 Z" fill="${col}"/>
             <rect x="${x - r - 1}" y="${y + 4}" width="${r * 2 + 2}" height="7" rx="3.5" fill="${p.light}"/>
             <circle cx="${x}" cy="${y - 12}" r="5" fill="${p.light}"/>`,
    shades: `<g fill="${p.ink}"><rect x="${x - r + 2}" y="${y + r * 0.6}" width="${r - 3}" height="10" rx="5"/>
             <rect x="${x + 1}" y="${y + r * 0.6}" width="${r - 3}" height="10" rx="5"/>
             <rect x="${x - 2}" y="${y + r * 0.6 + 3}" width="4" height="3"/></g>`,
    scarf: `<path d="M${x - r + 3} ${y + r * 1.7} h${r * 2 - 6} v9 h-${r * 2 - 6} Z" fill="${col}"/>
            <path d="M${x + r - 9} ${y + r * 1.7 + 9} l7 14 l-8 2 l-4 -14 Z" fill="${col}"/>`,
    halo: `<ellipse cx="${x}" cy="${y - 10}" rx="${r * 0.62}" ry="4.5" fill="none" stroke="${oklchToHex(toDisplayable([0.88, 0.15, 92]))}" stroke-width="3.5"/>`,
    crown: `<path d="M${x - 14} ${y + 2} l3 -16 l6 7 l5 -11 l5 11 l6 -7 l3 16 Z" fill="${oklchToHex(toDisplayable([0.85, 0.15, 88]))}"/>`,
  };
  return hats[kind] || '';
}

// Which hat the calendar suggests. Never applied unless asked for — a library
// that silently puts a Santa hat on your users in December is a support ticket.
export function seasonalHat(date) {
  const m = date.getMonth() + 1;
  if (m === 12 || m === 1) return 'beanie';
  if (m >= 6 && m <= 8) return 'shades';
  if (m === 2 || m === 3) return 'scarf';
  return 'party';
}

const RENDER = {
  face, pencil, specs, cat, ghost, sticker, line,
  dog, duck, capybara, monster, object,
  geometric, blob, pixel, rings, initials, isometric, bauhaus,
};

// Sketch styles want paper behind them; a saturated disc makes a pencil drawing
// look like clip art.
const ON_PAPER = new Set(['pencil', 'line']);
...
export function avatar(seed, options = {}) {
  const {
    ...
    label = null,
    mood = null, // see MOODS; only MOOD_STYLES react to it
    hat = 'none', // see HATS
  } = options;

  const chosen = RENDER[style] || style === 'duo' ? style : 'face';
  ...
  // The clip id has to be unique. These get composed into strips — a README
  // banner is twenty avatars in one document — and twenty elements sharing
  // id="c" means every one of them uses the first shape it finds.
  // Everything that distinguishes one render from another goes into the hash.
  // Two byte-identical avatars still share an id, which is fine — they clip to
  // the same shape, so there is nothing to get wrong.
  const clipId = `c${hash(`${seed}:${style}:${radius}:${size}:${mood}:${hat}:${on}`).toString(36)}`;
  const clip =
    radius === 'square'
      ? ''
      : radius === 'squircle'
        ? `<clipPath id="${clipId}"><rect x="0" y="0" width="100" height="100" rx="28"/></clipPath>`
        : `<clipPath id="${clipId}"><circle cx="50" cy="50" r="50"/></clipPath>`;

  const paper = ON_PAPER.has(chosen) ? oklchToHex(toDisplayable([0.96, 0.012, hueOf(p)])) : p.bg;
  const opts = { mood, hat };

  // `duo` isn't a drawing, it's an arrangement — it borrows another style and
  // renders it twice, which is why it can't live in the RENDER table.
  const drawn =
    chosen === 'duo'
      ? duo(seed, p, label, paper, (s, pal, l, bd) => {
          const pick = chance(`duopick:${seed}`).pick([face, specs, cat, ghost, pencil]);
          return pick(s, pal, l, bd, opts);
        })
      : RENDER[chosen](seed, p, label, paper, opts);

  const body = `
    <rect width="100" height="100" fill="${paper}"/>
    ${drawn}
    ${hatFor(hat, seed, p, chosen)}`;

  return (
    `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 100 100" ` +
    `role="img" aria-label="${esc(label || `Avatar for ${seed}`)}">` +
    (clip ? `<defs>${clip}</defs><g clip-path="url(#${clipId})">${body}</g>` : body) +
    ring +
    `</svg>`
  );
}
