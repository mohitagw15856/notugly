...
/**
 * The theme file alone isn't an extension. `vsce package`/`vsce publish`
 * expect a publisher, an icon, a README and a changelog — this fills in
 * every field that can be filled in from the system itself, and leaves an
 * honest placeholder (with instructions) for the two that can't be: the
 * publisher ID and the repository URL, which only the person shipping it has.
 */
// icon.mjs/raster.mjs depend on node:zlib, which does not exist in a
// browser. Everything else in this file is loaded by the website (through
// export.mjs's exportAll) at module-evaluation time — a static import here
// would drag node:zlib into that graph and break the site the moment
// anything imported targets.mjs, not just the one command that needs it. A
// dynamic import defers the fetch until this function actually runs, which
// only ever happens from the CLI.
export async function toVsCodeExtension(sys) {
  const [{ iconPixels }, { encodePng }] = await Promise.all([import('./node/icon.mjs'), import('./node/raster.mjs')]);

  const base = toVsCode(sys);
  const pkg = JSON.parse(base['package.json']);
  pkg.publisher = 'your-publisher-name';
  pkg.icon = 'icon.png';
  pkg.repository = { type: 'git', url: 'https://github.com/your-org/your-repo' };
  pkg.galleryBanner = { color: sys.colour.bg, theme: sys.dark ? 'dark' : 'light' };

  const letter = String(sys.seed).trim()[0] || 'N';
  const icon = encodePng(iconPixels(letter, { bg: sys.colour.buttonBg, size: 128 }));

  const readme = `# notugly ${sys.seed}

A VS Code colour theme generated by [notugly](https://github.com/mohitagw15856/notugly) — ${sys.vibeLabel}, seed \`${sys.seed}\`.

Every syntax colour clears 4.5:1 against the editor background. That isn't a claim on this README — it's checked on every commit in notugly's own test suite, the same way the rest of the palette is.

## Install

Extensions → \`...\` menu → Install from VSIX, or:

\`\`\`
code --install-extension notugly-${sys.seed}-1.0.0.vsix
\`\`\`

## Publish it yourself

This is a generated scaffold, not a submitted extension. Before running \`vsce package\` / \`vsce publish\`:

1. Register a publisher at https://marketplace.visualstudio.com/manage and put its ID in \`package.json\`'s \`publisher\` field (currently \`your-publisher-name\`).
2. Point \`repository.url\` at wherever this actually lives.

Everything else — the theme, the icon, this README — is already generated from the system it describes.
`;
...
Initial generated release — ${sys.vibeLabel}, seed \`${sys.seed}\`.
`;

  const vscodeignore = ['.vscode/**', '.vscode-test/**', '**/*.map', '.gitignore', '*.vsix'].join('\n') + '\n';

  return {
    'package.json': JSON.stringify(pkg, null, 2),
    'themes/notugly-color-theme.json': base['themes/notugly-color-theme.json'],
    'icon.png': icon,
    'README.md': readme,
