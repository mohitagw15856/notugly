...
import { shadow, allShadows, allElevationSurfaces, LEVELS as SHADOW_LEVELS } from './shadow.mjs';
import { motion } from './motion.mjs';
import { pattern } from './pattern.mjs';
import { gradient } from './gradient.mjs';
import { glassMaterial, glassLegibility, concentricRadius } from './liquidglass.mjs';
...
export function system(seed = 'notugly', { vibe = 'editorial', dark = false, brand: locked = null } = {}) {
  const v = getVibe(vibe);
  const c = chance(`sys:${seed}:${vibe}`);

  // Brand lock. The commonest and most reasonable objection to any generator is
  // "this is lovely but our colour is #E4002B and that is not negotiable". So
  // pin it: the locked hue becomes the primary and everything else — the
  // accent, the neutral tint, the ramps — is built around it. The seed still
  // controls the parts you don't care about.
  const lockedLch = locked ? hexToOklch(locked) : null;
  const hue = lockedLch ? lockedLch[2] : c.pick(SEED_HUES) + c.float(-14, 14);
  const accentHue = (hue + c.pick([150, 180, 210, -40])) % 360;

  const chroma = lockedLch ? lockedLch[1] : 0.15 * v.chroma;
  // A locked brand colour is used exactly as given. Anything else and you have
  // not really locked it.
  const primaryBase = locked || oklchToHex(toDisplayable([0.55, chroma, hue]));
  ...
  // The whole ladder, not just the rung this vibe stands on. The site shows an
  // elevation scale and the export has to actually contain it — otherwise the
  // panel is advertising something you cannot take away with you.
  const elevation = allShadows({ hue, dark });
  ...
  const built = {
    ...
    shadow: sh,
    elevation,
    elevationLevels: SHADOW_LEVELS,
    // In dark mode a darker shadow on a dark page is invisible, so elevation
    // lifts the surface instead. In light mode these are all the same colour
    // and the shadows do the work.
    elevationSurface: allElevationSurfaces(surface, { dark, text }),
    motion: mo,
    pattern: pat,
    gradient: grad,
  ...
  // Liquid Glass is checkable in a way the other vibes aren't: it's a
  // translucent material, so its "background" pairing is only known at
  // build time here as a worst-case sweep, not a single number.
  if (v.liquidGlass) {
    built.liquidGlass = {
      regular: glassMaterial(built, { variant: 'regular' }),
      clear: glassMaterial(built, { variant: 'clear' }),
      legibility: {
        regular: glassLegibility(built, { variant: 'regular' }),
        clear: glassLegibility(built, { variant: 'clear' }),
      },
      concentricRadius,
    };
  }
