...
// Single quotes, deliberately. A data URI wrapped in double quotes cannot be
// dropped into a double-quoted HTML style attribute — the attribute ends at the
// first quote and the browser silently turns the rest into junk attributes.
// The payload is percent-encoded, so it can never contain a single quote itself.
